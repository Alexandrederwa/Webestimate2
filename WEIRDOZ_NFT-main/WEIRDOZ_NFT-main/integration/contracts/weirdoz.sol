...
contract weirdoz is  Ownable, ERC721A, ReentrancyGuard {
    ...
    uint64 public PRICE = 0.2 ether;
    uint64 public WHITELIST_PRICE = 0.1 ether ;
	 ...
	 * Constructor
	 */
    constructor(string memory notRevealBaseURI) ERC721A("WEIRDOZ", "WEIR") {
    ...
    /// @param _merkleProof proof of member of a whitelist
    ...
    /// Public minting open to whitelist
	/// @dev mints tokens during whielist sale, limited by `MAX_PER_WHITELIST`
	/// @param _count number of tokens to mint in transaction
    /// @param _merkleProof proof of member of a whitelist
    function whitelistMintNFTs(bytes32[] calldata _merkleProof , uint _count) external payable callerIsUser {

        uint totalMinted = _tokenIds.current();
        require(whitelistClaimed[msg.sender] < MAX_PER_WHITELIST , "Address has already claimed.");
        ...
        uint256 totalCost = WHITELIST_PRICE * _count;
        _safeMint(msg.sender, _count);
        whitelistClaimed[msg.sender] = whitelistClaimed[msg.sender] + _count;
        refundIfOver(totalCost);
    }
	...
	/// @dev Reveal nft metadata
    function reveal() public onlyOwner {
        revealed = true;
    }

    /// @dev active public sale
    function activeSale() public onlyOwner {
        saleIsActive = true;
    }

    /// @dev active whitelist sale
